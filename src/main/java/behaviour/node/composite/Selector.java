...
public class Selector extends Composite<config.behaviour.node.Selector> implements IPreCondition {

    private Condition<? extends config.behaviour.Condition> preCond = null;
    private int curRunningIndex = 0;
    public Selector(BehaviourTree behaviourTree, Node nodeCfg) {
    ...
    private Statusenum _update(BehaviourStack stack, Statusenum childStatus) {
        ...
        while (true) {
            if (runningStatus == Statusenum.BTSUCCESS) {
